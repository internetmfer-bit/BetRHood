...
`betrhood.com` — raw file links (what the gateway serves). `app.betrhood.com` — the website.
...
- **`gateway/`** — Cloudflare Worker (lives at the bare `betrhood.com`) that turns a link like
  `betrhood.com/<address>/<key>` into an HTTP response, so anyone can open a link in a plain
  browser with no wallet required. See its own README for local dev and deploy instructions.
- **`frontend/`** — the web app (lives at `app.betrhood.com`): homepage/forum, upload,
  profile, link viewer.
